...
export default function Transport() {
  ...
  const [_modeOfTravelForDropOptions, setModeOfTravelForDropOptions] = useState(
    [],
  );
  const [filterModeOfTravelForDrop, _setFilterModeOfTravelForDrop] =
    useState('');

  const [searchQuery, setSearchQuery] = useState('');
  const [filterNeedForPickup, _setFilterNeedForPickup] = useState('');
  const [filterNeedForDrop, _setFilterNeedForDrop] = useState('');

  // IDs for certain components
  const arrivalDateInputId = useId();
  const departureDateInputId = useId();
  const searchInputId = useId();

  const districtSelectId = useId();
  const eventSelectId = useId();
  const groupSelectId = useId();
  const modeOfTransportSelectId = useId();
      ...
      <div className="flex flex-row m-4 gap-4 items-center">
        <div className="bg-white p-4 rounded-2xl border grow">
          <div className="flex flex-col gap-2">
            <label htmlFor={modeOfTransportSelectId}>
              <b>Mode Of Transport</b>
            </label>
            <select
              id={modeOfTransportSelectId}
              className="border p-2 rounded-2xl"
        ...
        <div className="bg-white p-4 rounded-2xl border grow">
          <div className="flex flex-col gap-2">
            <label htmlFor={arrivalDateInputId}>
              <b>Arrival Date</b>
            </label>
            <input
              type="date"
        ...
        <div className="bg-white p-4 rounded-2xl border grow">
          <div className="flex flex-col gap-2">
            <label htmlFor={departureDateInputId}>
              <b>Departure Date</b>
            </label>
            <input
              type="date"
          ...
          <div className="flex justify-around gap-3">
            ...
            <div className="pt-2">
              ...
              <p className="text-4xl font-bold">
                {filteredData.reduce(
                  (acc, row) =>
                    acc + parseInt((row.gender === 'Female' ? 1 : 0) ?? 0, 10),
                  0,
                )}
              </p>
            </div>
